    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                ...
                if tipo == TipoTransaccionEnum.COMPRA.value:
                ...
                # Para VENTAS (crédito), el flujo sigue como antes
                    ...
                    if requiere_pago_tarjeta(transaccion):
                        try:
                            checkout_url = crear_checkout_para_transaccion(transaccion)
                            return HttpResponseRedirect(checkout_url)
                        except Exception as e:
                            logger.exception("Error al crear checkout de Stripe para tx #%s", transaccion.id)
                            messages.error(request, f"No se pudo iniciar el pago con tarjeta: {e}")
                            return redirect("transacciones:transacciones_list")
                ...
                logger.exception("Error inesperado en transaccion_create")
    ...
    if request.method == 'POST':
        ...
        if action == 'resend':
            try:
                ...
                messages.error(request, str(e))
            return render(request, 'transacciones/verify_otp.html', {'email': pending_tx['email_mfa']})
        ...
        try:
            ...
            if is_valid:
                ...
                messages.success(request, f"Transacción de compra {transaccion.id} creada y confirmada exitosamente.")
                return redirect('transacciones:transacciones_list')

        except ValidationError as e:
            messages.error(request, str(e))

    return render(request, 'transacciones/verify_otp.html', {'email': pending_tx.get('email_mfa')})
